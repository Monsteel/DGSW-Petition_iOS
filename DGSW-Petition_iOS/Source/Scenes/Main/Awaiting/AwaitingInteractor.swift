        ...
        petitionWorker?.getPetitions(0, Constants.INFINITE_SCROLL_LIMIT, type: .AWAITING) { [weak self] in
            switch $0 {
                ...
                case .failure(let err):
                    self?.presentInitialView(nil, nil, err.toAwaitingError(.FailAwaitingPetition))
            }
        }
    }
...
extension AwaitingInteractor {
    private func presentInitialView(_ petitionSimpleInfos: [PetitionSimpleInfo]?, _ categoryInfos: [CategoryInfo]?, _ error: AwaitingError?){
